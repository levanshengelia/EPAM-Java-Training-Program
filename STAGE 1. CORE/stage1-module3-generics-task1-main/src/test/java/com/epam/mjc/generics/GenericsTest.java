...
public class GenericsTest {
    ...
    public void testBoxingMethodIsNotRaw() {
        ...
        assertTrue("In line " + (list.indexOf(getLineWithString(list, "boxingMethod")) + 1) + ", List shouldn't be raw", getLineWithString(list, "boxingMethod").contains("List<List<String>>"));
        assertTrue("In line " + (list.indexOf(getLineWithString(list, "firstList =")) + 1) + ", List shouldn't be raw", getLineWithString(list, "firstList =").contains("List<String>"));
        assertTrue("In line " + (list.indexOf(getLineWithString(list, "secondList =")) + 1) + ", List shouldn't be raw", getLineWithString(list, "secondList =").contains("List<String>"));
    ...
    public void testGenericMethodUseGenerics() {
        List<String> list = parseGenericsClass();
        assertTrue(" genericMethod should use generics", getLineWithString(list, "genericMethod").contains("<"));
        assertTrue(" genericMethod should use generics", getLineWithString(list, "genericMethod").contains(">"));
    }
    ...
    public void testGenericCloneHasIsNotRaw() {
        List<String> list = parseGenericsClass();
        assertTrue("In line " + (list.indexOf(getLineWithString(list, "clone")) + 1) + ", List shouldn't be raw", getLineWithString(list, "clone").contains("List<? super T>"));
        assertTrue("In line " + (list.indexOf(getLineWithString(list, "clone")) + 1) + ", List shouldn't be raw", getLineWithString(list, "clone").contains("List<? extends T>"));
    }
